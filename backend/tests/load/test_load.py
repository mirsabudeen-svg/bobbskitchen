...
class TestDTFThroughputModel:
    ...
    Assumptions (adjust to your actual press specs):
        ...
        - Operating day: 8 hours (480 min) — BOBB roaming van
        - 60 orders spread across the operating day → 1 order every 8 min
        - Journey promise: order ready within 14 minutes of placement
    """

    PRESS_TIME_MINUTES        = 5.0
    OPERATING_MINUTES         = 480.0   # 8-hour operating day
    ORDERS_PER_DAY            = 60
    ACCEPTABLE_WAIT_MINUTES   = 14.0    # per-customer journey promise

    def test_single_press_can_fulfil_journey_volume(self):
        """
        Simulate order arrivals as a simple D/D/1 queue over a full operating day.
        Assert median wait stays under the 14-minute journey promise.
        """
        arrival_gap = self.OPERATING_MINUTES / self.ORDERS_PER_DAY

        arrival_times = [i * arrival_gap for i in range(self.ORDERS_PER_DAY)]
        press_free_at = 0.0
        wait_times = []
        ...
        assert median_wait <= self.ACCEPTABLE_WAIT_MINUTES, (
            ...
            f"at {self.ORDERS_PER_DAY} orders over {self.OPERATING_MINUTES:.0f} min. "
