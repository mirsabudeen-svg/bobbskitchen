...
type Order = Pick<StaffOrder, 'order_id' | 'short_ref' | 'order_status' | 'customer_phone' | 'whatsapp_sent'>;
...
export function WhatsAppStatus({ order, onRetry }: Props) {
  ...
  const fetchLogs = async () => {
    const r = await fetch(`/api/v1/orders/${order.order_id}/whatsapp-log`);
    if (r.ok) {
  ...
  const handleRetry = async () => {
    setRetrying(true);
    try {
      const r = await fetch(`/api/v1/orders/${order.order_id}/whatsapp-retry`, {
        method: "POST",
      });
      if (r.ok && onRetry) {
